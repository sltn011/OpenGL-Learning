...
std::unique_ptr<glm::mat4[]> generateTranslationMatrices(size_t numMatrices, glm::vec3 center) {
    ...
    for (size_t i = 0; i < numMatrices; ++i) {
        ...
        float y = displacement * 0.75f + (rand() % 30) - 15;
        displacement = (std::rand() % static_cast<int>(2 * 100 * offset)) / 100.0f - offset;
...
(
    ...
    OGL::Object asterodiObj(asteroid);

    size_t numInstances = 15000;

    std::unique_ptr<glm::mat4[]> asteroidTranslationMatrices = generateTranslationMatrices(numInstances, planetCenter);
    ...
    for (size_t i = 0; i < 1; ++i) { // Asteroid is 1 mesh only
        glBindVertexArray(asteroid.meshVAO(i));
        for (int j = 0; j < 4; ++j) {
            glEnableVertexAttribArray(3 + j);
            glVertexAttribPointer(3 + j, 4, GL_FLOAT, GL_FALSE, 4 * sizeof(glm::vec4), (void*)(j * sizeof(glm::vec4)));
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        asterodiObj.drawInstanced(shaderAsteroid, numInstances);
