...
void main() {
	...
	case 1: // Reflection
		reflected = reflect(viewDir, norm);
		reflected.z *= -1; // RHS -> LHS because of skybox
		fragColor = vec4(texture(skyboxSampler, reflected).rgb, 1.0);
		break;

	case 2: // Refraction
		ratio = 1.0 / 1.52;
		refracted = refract(viewDir, norm, ratio);
		refracted.z *= -1; // RHS -> LHS because of skybox
		fragColor = vec4(texture(skyboxSampler, refracted).rgb, 1.0);
		break;

	case 3: // Attempt to combine reflection and refraction :b
		reflected = reflect(viewDir, norm);
		reflected.z *= -1; // RHS -> LHS because of skybox
		ratio = 1.0 / 1.52;
		refracted = refract(viewDir, norm, ratio);
		refracted.z *= -1; // RHS -> LHS because of skybox
		vec3 color = 2 * (texture(skyboxSampler, reflected).rgb) / 10.0 + 8 * (texture(skyboxSampler, refracted).rgb) / 10.0;
